...
class MultiFold():
    ...
    def RunModel(self,
        ...
        for e in range(self.n_ensemble):
            ...
            self.CompileModel(model_e, num_steps, stepn=stepn)
    ...
    def CompileModel(self,model,num_steps,fixed=False, stepn=None):

        lr_step1 = self.LR
        lr_step2 = self.LR / 5.0

        min_lr_step1 = 1e-5
        min_lr_step2 = 1e-5 / 5.0

        if stepn == 1:
            lr = lr_step1 if not fixed else min_lr_step1
        elif stepn ==2:
            lr = lr_step2 if not fixed else min_lr_step2
        else:
            lr = self.LR if not fixed else 1e-5
    ...
    def CompileModels(self,fixed=False):
        ...
        self.CompileModel(self.model1,self.train_frac*self.num_steps_reco,fixed, stepn=1)
        self.CompileModel(self.model2,self.train_frac*self.num_steps_gen, fixed, stepn=2)

        if self.n_ensemble > 1 and len(self.step1_models) > 0:
            for model in self.step1_models:
                self.CompileModel(model,self.train_frac*self.num_steps_reco,fixed, stepn=1)
            for model in self.step2_models:
                self.CompileModel(model,self.train_frac*self.num_steps_gen, fixed,stepn=2)
